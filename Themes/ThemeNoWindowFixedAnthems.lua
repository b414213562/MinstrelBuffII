ThemeNoWindowFixedAnthems = class();

ThemePath4 = ThemesPath .. "4 No Window Fixed Anthems/";

function ThemeNoWindowFixedAnthems:Constructor()
	self.name = "No Window - Fixed anthems"; -- Name of the theme

	-- The whole window.
	self.windowBackground = ThemePath4 .. "minstrel_buffbar_background_clear_fixed.tga"; -- The background for the window. Use it, when you have a combined picture of ballads and anthems. Can be nil for no image.
	self.windowWidth = 210; -- The size of the window.
	self.windowHeight = 104; -- The size of the window.
	...
	self.anthemBorder = ThemesPath .. "mb_anthem_border.tga"; -- The background for the anthem border. Can be nil for no image. Only used, when fixedAnthems is true.
	...
	-- Priority anthem
	self.priorityAnthemX = 147;
	self.priorityAnthemY = 3;

	-- Combat timer
	self.combatTimerX = 28;
	self.combatTimerY = 100;
	self.combatTimerHeight = 3;
	self.combatTimerWidth = 156;
end