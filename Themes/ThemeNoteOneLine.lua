...
ThemeNoteOneLine = class();

ThemePath11 = ThemesPath .. "11 Note One Line/";

function ThemeNoteOneLine:Constructor()
	self.name = "Note theme - One line"; -- Name of the theme
	...
	self.anthemBackground = ThemePath11 .. "mb_anthem_note_background.tga"; -- The background for the anthem area. Can be nil for no image.
	...
	self.anthemPosX = 154; -- The position for the anthem area relative to the window.
	self.anthemPosY = 0; -- The position for the anthem area relative to the window.
	self.anthemWidth = 38; -- The size of the anthem area.
	self.anthemHeight = 67; -- The size of the anthem area.
	self.anthemEffectStartX = 155; -- The first anthem image starts here. Relative to the window.
	self.anthemEffectSpaceX = 39; -- The space between the anthem images from the left (must include the width).
	self.anthemEffectY = 18; -- The anthem images space from top relative to the window.
	self.anthemEffectWidth = 28; -- The size of the anthem images.
	self.anthemEffectHeight = 28; -- The size of the anthem images.
	
	-- Anthem timers
	self.anthemTimeStartX = 156; -- The first anthem timer starts here. Relative to the window
	self.anthemTimeSpaceX = 39; -- The space between the anthem timers from the left (must include the width).
	self.anthemTimeY = 53; -- The anthem timer space from top relative to the window.
	self.anthemTimeWidth = 24; -- The size of the anthem timer.
	self.anthemTimeHeight = 6; -- The size of the anthem timer.

	-- Priority anthem
	self.priorityAnthemX = 192;
	self.priorityAnthemY = 14;

	-- Combat timer
	self.combatTimerX = 38;
	self.combatTimerY = 65;
	self.combatTimerHeight = 3;
	self.combatTimerWidth = 150;
